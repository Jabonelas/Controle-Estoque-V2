    ...
    public partial class VendasRelatorio : Form
    {
        ...
        private void btnPesquisa_CadastroCliente_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (int Cod_Produto in BRSG.BuscarDadosRelatorioNFSaidaGraficoCod_Produto(Convert.ToDateTime(dtpDataInicio_Relatorio.Text),
                Convert.ToDateTime(dtpDataFinal_Relatorio.Text)))
                {
                    foreach (int QUANT in BRSG.BuscarDadosRelatorioNFSaidaGraficoQuant(Convert.ToDateTime(dtpDataInicio_Relatorio.Text),
